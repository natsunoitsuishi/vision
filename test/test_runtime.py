...
#             cameras: Dict[int, OptCameraClient],  # 改为 int key
...
#         ts = event.payload.get("timestamp", time.time())
#
#         if sensor == "PE1":
#             # 创建新轨迹
...
#                 sensor_distance = get_config("trigger.sensor_distance_mm", 120)
#                 if time_diff > 0:
#                     track.speed_mm_s = sensor_distance / time_diff
#                 else:
#                     track.speed_mm_s = get_config("runtime.line_speed_mm_s", 800)
#
#             # 打开扫描窗口
...
#
#         # 从 payload 构建 CameraResult
#         from domain.models import CameraResult
#         camera_result = CameraResult(
#             camera_id=camera_id,
#             result=payload.get("result", "NG"),
...
#                          f"code={camera_result.code}, success={camera_result.is_success}")
#
#         # 解析最终结果（如果已满足判定条件）
...
#         })
#
#     async def _notify_ui_camera_status(self, camera_id: int, status: str) -> None:
#         """通知 UI 相机状态"""
#         self.event_bus.emit(EventType.UI_UPDATE, "runtime", {
#             "type": "camera_status",
#             "camera_id": camera_id,
