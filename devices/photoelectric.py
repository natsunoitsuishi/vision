...
class PhotoelectricClient:
    def __init__(self, event_bus: EventBus):
        ...
        # 连接参数
        self.host = get_config("photoelectric.host")
        self.port = get_config("photoelectric.port")
        self.timeout = get_config("photoelectric.timeout")
    ...
    async def main():
        ...
        # 一次读取 DI1 + DI2 两个光电（地址0、地址1，共2个点）
            ...
            if not result.isError():
                ...
                if pe1:
                    print(f"光电1状态: {pe1}                      , 时间: { format_ms( time.time_ns() / 1_000_000 )}")
                if pe2:
                    print(f"                   |  光电2状态: {pe2}, 时间: { format_ms( time.time_ns() / 1_000_000 )}")
            else:
                print("读取失败")
